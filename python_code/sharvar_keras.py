"""
sharvar_keras.py: Uses a neural networking approach to solve the cartpole environment and generate training data
https://github.com/sharvar/gym/blob/master/cartpole_train_ppo_v2.py
...
ts = time.time()
dir_path = "data/sharvar_keras/" + str(str(datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d_%H_%M_%S')))
            ...
            if done:
                ...
                if self.episode_reward > 200:
                    ...
                    np.savetxt(dir_path + "/" + str(datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d_%H_%M_%S'))
                               + f"#{self.episode_reward}-record.csv", record[:100], delimiter=',', fmt='%.4f')
